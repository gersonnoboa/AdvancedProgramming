...
let reverses xs ys =
    let rxs = List.rev xs
    let rys = List.rev ys
    let individualReverse = List.concat [rxs;rys]

    let combined = List.concat [ys;xs]
    let combinedReverse = List.rev combined

    individualReverse = combinedReverse

Check.Quick reverses
...
let checkPalindrome (xs: int list) = 
    //let palindromeLists = Arb.from<list<int>> |> Arb.filter (fun xs -> xs = List.rev xs)
    let filter xs' = xs' = List.rev xs'
    //Prop.forAll palindromeLists (fun xs -> isPalindrome xs)
    filter xs ==> isPalindrome xs
    
Check.Quick checkPalindrome

let checkPalindrome' xs = 
    checkPalindrome xs |> Prop.collect (List.length xs)
...
let palindromeGenerator = 
    Gen.map (fun (x: int list) -> toPalindrome x) Arb.generate<list<int>>

let checkForPalindrome =
    (Prop.forAll (Arb.fromGen palindromeGenerator) (fun palindrome -> isPalindrome palindrome))

Check.Quick checkForPalindrome

let checkForPalindrome' xs =
    checkForPalindrome |> Prop.collect(List.length xs)
